...
case "$1" in
  install)
    ...
    ;;
  start)
    "$docker" run -d -p 0.0.0.0:7903:7903 --env-file=env-mainnet --name p2pool-dash thelazier/docker-p2pool-dash:latest
    ;;
  stop)
    "$docker" stop p2pool-dash
    ;;
  start-testnet)
    "$docker" run -d -p 0.0.0.0:17903:17903 --env-file=env-testnet --name p2pool-dash-testnet thelazier/docker-p2pool-dash:latest
    ;;
  stop-testnet)
